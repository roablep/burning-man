...
from ._nextgen_common import read_csv, tokenize, write_csv

DEFAULT_OUTPUT_DIR = "reports/field_notes/next_gen"
BASE_FILENAME = "nextgen_pooled_base.csv"
DELTA_FILENAME = "nextgen_theme_deltas.csv"
SUMMARY_FILENAME = "nextgen_theme_deltas.md"
...
def run_analysis(input_csv: str, output_dir: str = DEFAULT_OUTPUT_DIR) -> tuple[str, str]:
    ...
    for tenure in ["Virgin", "Sophomore", "Veteran", "Elder"]:
        ...
        if len(ng_tenure) < 20 or len(ng_other) < 20:
            continue
        all_rows.extend(
            build_delta_rows(ng_tenure, ng_other, segment=f"nextgen_{tenure}_vs_rest", top_n=30)
        )
