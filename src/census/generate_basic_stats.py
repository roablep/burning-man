...
def generate_ascii_histogram(data, title, width=50):
    if not data: return f"No data for {title}"
    
    counts = Counter(data)
    if not counts: return f"No data for {title}"
...
def analyze_file(filename, year):
    ...
    with open(filename, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            if age_str: ages.append(int(age_str))
            
            # Gender
            g = row.get("Norm_Gender")
            if g and g != "U": genders.append(g)
            ...
            if s and s != "Unknown": statuses.append(s)
            
            # Burn Count (for numeric stats)
            bc = row.get("Norm_Burn_Count")
            if bc: burn_counts.append(int(bc))

    report = []
    report.append(f"## Year {year} Analysis")
    report.append(f"**Total Responses:** {len(ages)}") # Approximation based on age, or row count
    ...
    binned_ages = []
    for a in ages:
        if a < 10: binned_ages.append("0-9")
        elif a >= 80: binned_ages.append("80+")
        else: binned_ages.append(f"{int(a/10)*10}-{int(a/10)*10+9}")
